...
* `ardupilot`
  ...
  * Default entrypoint starts `gazebo-iris` model with added static down (FRD frame) facing camera at
    KSQL Airport.
* `px4`
  * PX4 Gazebo SITL simulation
  * Default entrypoint starts `typhoon_h480` model at KSQL Airport
* `mavros`
  * MAVROS (used for ArduPilot SITL)
* `micro_ros_agent`
  * micro-ROS-Agent (used for PX4 SITL)
* `mapserver`
...
* `gisnav`
  ...
  * Launches GISNav with PX4 configuration by default (intended to be used with `px4` service)
...
> **Note** Run the below commands without the `gisnav` service if you want to develop with or perform automated 
> testing on a local copy of `gisnav`
...
```bash
docker compose build mapserver px4 micro-ros-agent gisnav
```
...
```bash
docker compose up mapserver px4 micro-ros-agent gisnav
```
...
docker compose \
  ...
  build mapserver ardupilot mavros
```
...
docker compose up mapproxy px4 micro-ros-agent gisnav
```
...
If you are not able to establish ROS communication between the `mavros` or `micro-ros-agent` container and the host and 
receive the above error, try disabling Fast DDS on your host. You can do so by creating an XML configuration (e.g. 
`disable_fastrtps.xml`) as described in [this comment][11] and restarting ROS 2 daemon with the new configuration:
